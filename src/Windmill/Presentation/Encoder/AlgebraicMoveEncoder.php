...
class AlgebraicMoveEncoder implements MoveEncoderInterface
{
    ...
    public function encode(AbstractMove $move, Game $game): string
    {
        switch ($move::class) {
            ...
            case MultiMove::class:
                ...
                if ($movingPiece::class == King::class && abs($move->from[0]->file() - $move->to[0]->file()) > 1) {
                    // castling
    ...
    public function decode(mixed $algebraic, Game $game): AbstractMove
    {
        ...
        if (sizeof($possibleMoves) <> 1) {
            throw new Exception(sprintf(
                "Expected exactly one possible move that results into '%s', got %d%s",
                $algebraic,
                sizeof($possibleMoves),
                sizeof($possibleMoves) > 0 ? sprintf(' (%s)', implode(',', array_keys($possibleMoves))) : ''
            ));
        }
